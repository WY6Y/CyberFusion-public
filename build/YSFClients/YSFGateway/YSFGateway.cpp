...
// 0 = disable poll-lost timeout (local parrot only answers polls; link must stay up)
m_lostTimer(1000U, 0U),
	...
	while (!m_killed) {
		...
		if (m_lostTimer.isRunning() && m_lostTimer.hasExpired()) {
			if (m_linkType == LINK_TYPE::YSF) {
				...
				if (reconnect && !m_current.empty()) {
					reconnectReflector("poll_lost", m_current);
				} else {
